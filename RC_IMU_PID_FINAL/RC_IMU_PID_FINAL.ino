...
volatile double  Output_roll;
volatile double errSum_roll, lastErr_roll;
volatile double kp_roll=0.02;
volatile double ki_roll=0;
volatile double kd_roll=0;

volatile unsigned long lastTime_pitch;
volatile double Input_pitch, Output_pitch, Setpoint_pitch;
volatile double errSum_pitch, lastErr_pitch;
volatile double kp_pitch=0.02;
volatile double ki_pitch=0;
volatile double kd_pitch=0;


volatile unsigned long lastTime_yaw;
volatile double Input_yaw, Output_yaw, Setpoint_yaw;
volatile double errSum_yaw, lastErr_yaw;
volatile double kp_yaw=0.08;
volatile double ki_yaw=0;
volatile double kd_yaw=0;
...
//############################################################################
  ...
  //******************************PID CALC***************************************// 
  //******************************PID CALC***************************************// 
  Compute_pitch((pitchValue/4),pitch1);
  Compute_roll((rollValue/4),roll1);
  Compute_yaw(0,yaw1);
  ...
  if((bUpdateFlags & THROTTLE_FLAG))
  {
   ...
   {
      Brushless_1.writeMicroseconds(unThrottleIn-(Output_pitch*unThrottleIn*0.05)-(Output_yaw*unThrottleIn*0.05));
      Brushless_2.writeMicroseconds(unThrottleIn+(Output_roll*unThrottleIn*0.05)+(Output_yaw*unThrottleIn*0.05));
      Brushless_3.writeMicroseconds(unThrottleIn+(Output_pitch*unThrottleIn*0.05)-(Output_yaw*unThrottleIn*0.05));
      Brushless_4.writeMicroseconds(unThrottleIn-(Output_roll*unThrottleIn*0.05)+(Output_yaw*unThrottleIn*0.05));
   bUpdateFlags = 0;
   
   }
  else if(unThrottleIn<=1200)
  {
      Brushless_1.writeMicroseconds(1000);
      Brushless_2.writeMicroseconds(1000);
      Brushless_3.writeMicroseconds(1000);
      Brushless_4.writeMicroseconds(1000);
     bUpdateFlags = 0;
...
//**********************************PID Program interrupt*************************//
//**********************************PID Program interrupt*************************//
//**********************************PID Program interrupt*************************//
//#################################################################################


void Compute_roll(double Setpoint,double Input)
{
   /*How long since we last calculated*/
   unsigned long now = millis();
   double timeChange = (double)(now - lastTime_roll);
   ...
   /*Compute PID Output*/
   Output_roll = kp_roll * error; //+ ki * errSum + kd * dErr;
...
void Compute_pitch(double Setpoint,double Input)
{
   /*How long since we last calculated*/
   unsigned long now = millis();
   double timeChange = (double)(now - lastTime_pitch);
   ...
   /*Compute PID Output*/
   Output_pitch = kp_pitch * error; //+ ki * errSum + kd * dErr;
...
void Compute_yaw(double Setpoint,double Input)
{
   /*How long since we last calculated*/
   unsigned long now = millis();
   double timeChange = (double)(now - lastTime_yaw);
   ...
   /*Compute PID Output*/
   Output_yaw = kp_yaw * error; //+ ki * errSum + kd * dErr;
